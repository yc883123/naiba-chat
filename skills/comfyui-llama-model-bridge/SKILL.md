...
description: 让 ComfyUI 模型目录中的 GGUF 主模型与视觉 mmproj 在 llama.cpp、Ollama、LM Studio 和 Naiba-chat 之间安全互通。用于配置或排查共享模型目录、本地 API、硬链接导入、Modelfile、视觉能力识别、模型迁移、重复占用磁盘、运行端选型与效率问题。
---

# ComfyUI 模型与 Llama 模型互通

让 llama.cpp、Ollama、LM Studio 和 Naiba-chat 尽量复用用户已有的 ComfyUI GGUF 模型，并正确处理视觉投影文件。

## 工作流程

1. 确认 ComfyUI 的模型根目录和用户希望接入的运行端。
2. 运行 `python scripts/inspect_model_compatibility.py --root <模型目录>` 做只读盘点，区分主模型与 `mmproj`、`vision projector` 等视觉投影文件。需要机器可读结果时添加 `--json`。
