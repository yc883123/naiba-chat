...
Supports all 374 RunningHub endpoints: image, video, audio, 3D, text understanding.
...
SITES = {
    "ai": {
        ...
        "account": "https://www.runninghub.ai/uc/openapi/accountStatus",
        "home": "https://www.runninghub.ai",
        "manage": "https://www.runninghub.ai/enterprise-api/consumerApi",
        "recharge": "https://www.runninghub.ai/vip-rights/4",
    },
    "cn": {
        "label": "CN site (China, runninghub.cn)",
        ...
        "manage": "https://www.runninghub.cn/enterprise-api/consumerApi",
        "recharge": "https://www.runninghub.cn/vip-rights/4",
    },
}
...
def read_site_from_config() -> str | None:
    """Read the preferred site (ai/cn) from openclaw config, if present."""
    ...
    site = entry.get("site")
    if isinstance(site, str) and site.strip().lower() in SITES:
        return site.strip().lower()
    env_val = entry.get("env", {}).get("RUNNINGHUB_SITE")
    if isinstance(env_val, str) and env_val.strip().lower() in SITES:
        return env_val.strip().lower()
    return None


def current_site() -> str:
    """Active site: --site / env RUNNINGHUB_SITE > openclaw config site > default 'ai'."""
    val = os.environ.get("RUNNINGHUB_SITE", "").strip().lower()
    if val in SITES:
        return val
    cfg_val = read_site_from_config()
    if cfg_val:
        return cfg_val
    return "ai"


def reload_site_config() -> None:
    """Recompute module-level URLs based on the active site (env or --site)."""
    global SITE, BASE_URL, ACCOUNT_STATUS_URL, HOME_URL, MANAGE_URL, RECHARGE_URL
    SITE = current_site()
    cfg = SITES[SITE]
    BASE_URL = cfg["api"]
    ACCOUNT_STATUS_URL = cfg["account"]
    HOME_URL = cfg["home"]
    MANAGE_URL = cfg["manage"]
    RECHARGE_URL = cfg["recharge"]
...
def require_api_key(provided_key: str | None) -> str:
    ...
    result = {
        ...
        "steps": [
            f"1. Register/login at {HOME_URL}",
            f"2. Create API Key at {MANAGE_URL}",
            f"3. Recharge wallet at {RECHARGE_URL}",
...
""",
    ...
    parser.add_argument("--site", choices=["ai", "cn"], default=None,
                        help="RunningHub site: ai (international runninghub.ai, default) or cn (runninghub.cn)")
    ...
    if args.site:
        os.environ["RUNNINGHUB_SITE"] = args.site
    reload_site_config()
