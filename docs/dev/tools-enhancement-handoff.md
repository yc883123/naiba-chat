...
- **分支**：`master`，HEAD = **`76b84a4`**（Harness 别名从模型可见集隐藏 + 影子旧类清理）；
- **验证基线**：`python -m unittest discover -s tests` = **155 用例 OK**；`tests.golden_replay` = 3 基线 OK；`.tmptest/scan_undef_all.py` = 0 候选；工具注册 def 总数 = **32**（模型/Web 可见 `schemas()` = **27**）；
...
3. 浏览器冒烟（Playwright）需用户本机 `npm install playwright` + Edge 权限，本 DSH 沙箱被拦（已知）。
4. `docs/manual/`（手动文档与截图）当前按用户指示不再维护（旧图含已移除的自动路由行，保留现状）。
...
- 后端（视重构/视觉/协议/配置/工具/会话）：`naiba/{vision/runtime,run/chat,run/stream,run/session,run/manager,core/contracts,tools/registry,config,app}.py`
- 前端：`public/{index.html,styles.css,js/09-settings.js,js/11-run-stream.js,js/12-chat-input.js}`
- 测试：`tests/{test_config_migration,test_vision_prepare_history,test_tool_registry_shape}.py`
- 发布/文档：`release_notes.json`、`README.md`、`项目维护说明（修改代码前必读）.md`、`docs/dev/tools-enhancement-handoff.md`（本文档）、`版本更新说明.md`（本地，gitignored）。**`docs/manual/` 按用户指示不再维护。**
- 验证脚本（gitignored）：`.tmptest/esm_graph_check.py` 已修复（剥离注释/字符串/模板/正则后的词边界扫描，消除正则字面量误报；合成用例验证真缺 import 仍能检出）

## 七、交接结语

工具系统统一架构、增强、视觉驱动化与别名隐藏已全部落地并通过验证；run/manager 拆分遗留（影子类）已清理。当前没有任何已知未修 bug。剩余工作均为增量优化与发布动作。接手第一步：`git status` 确认干净 + 跑 §五 全量测试（30 秒）确认基线。
