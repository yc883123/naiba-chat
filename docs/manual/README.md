# Naiba Chat 应用说明书（2.3.5 Beta）

> 本说明书面向「第一次打开 Naiba Chat」的小白读者，配图全部来自真实运行界面（2.3.5 Beta · 源码模式 · 截图 2026-09-12）。
...
1. **🆕 AI 回复能「重新生成」，自己的消息能「编辑」**：鼠标移到任意一轮上——AI 回复右下角是「复制 · **重新生成** · 新会话」，自己的消息右下角是「**编辑** · 分支」。点「重新生成」就拿原来那句提问再答一次；点「编辑」则在原处就地改字重开，`/ref` 与 `@` 引用、这一轮的附件全都跟着走。
2. **🆕 手机端功能对等 + 顶栏可折叠**：手机不再少半截——文件面板变**全屏抽屉**、「本轮修改的文件」恢复可点、桌面刻度轨换成顶栏**「轮次」下拉**，触摸目标统一 ≥44px；顶栏还能一键收起，把被它占掉的高度还给会话区。
3. **🆕 Skill 删错了能救回来**：删 Skill 不再是「一点就没」——先弹确认框让你选「移到回收目录（可恢复）」还是「彻底删除」，设置页的回收目录里能一键恢复。
4. **每个会话单独选模型**：顶栏选「API」（去哪儿），输入区选「模型」（派谁干），**按会话记忆、分支继承**，换完还自动重新探测图片能力；下拉末尾固定一个「**↻ 重新检测模型**」，供应商上新模型立刻可选。
5. **长会话不会越聊越贵**：聊到上下文快满时划一条「新会话」分割线，线以上的老消息不再发给模型，**聊天记录一条不删**，还能让模型自己写交接文档再重置。
6. **一个不够就接一堆模型**：OpenAI 兼容 / Anthropic / Gemini / Ollama / LM Studio / llama.cpp / Unsloth……在线本地都能接。
7. **Skill：给 AI 装"行业操作手册"**：写小说、跑 ComfyUI、写视频提示词……用 `/` 一键把领域知识塞进对话。
8. **MCP：给 AI 插外设**：ComfyUI、文件、网页……只要 MCP 服务器在跑，AI 就能直接调用。
9. **ComfyUI 自动流水线**：批改工作流 → 批量提交 → 自动收图 → 缩略图预览 → 一句话复用到下一轮。
10. **RunningHub 云端生成**：没显卡也能出 420+ 类型的图、视频、音频、3D。
11. **并行与分身**：后台 Job + 子 Agent，任务互不阻塞，跑图的时候还能写稿。
12. **手机/局域网远程访问**：同 WiFi 打开链接，躺床上也能用家里电脑。
13. **稳如老狗**：断线重连不重放、失败不丢已生成内容、5xx/429 自动重试、SHA-256 校验更新、可版本回退。
14. **文件面板 + 本轮修改文件**：AI 改过的文件直接在右侧看、可编辑、可存回磁盘，**改了什么一目了然**。
...
1. 去 GitHub Releases 页面下载 `naiba-chat-2.3.5-beta-windows-x64.zip`。
...
> 已经没有「轻量 / 富文本」开关了——**AI 能用哪些工具完全由 Agent 的工具集决定**，不再有会话级开关。想收窄能力，去「设置 → Agent」里改工具集。
>
> 这个控件从「平铺四段」改成了上拉框，省出的横向空间留给工作区和模型选择器；窄屏也不会被挤变形。
...
| 「把 build_info.json 的 version 改成新版本号」 | `read_file` + `edit_file` |
...
- **PDF 三件套**：`read_pdf` 抽文本层、`pdf_render_pages` 把页面渲成图、`pdf_zoom_region` 局部高清放大看小字和表格。扫描件（没文本层）会自动引导转页图，再让视觉模型识图。
...
**后台跑着也能继续聊天**：以前只要有一个长跑或卡住的异步任务，这个会话再发消息就会被拒掉（界面显示「当前对话已有 Run，已恢复其进度」，消息其实没入队）。现在**只有顶层对话本身占住互斥位**，后台 Job 不再阻塞下一轮——跑图的时候照样能接着问它有别的事。

**任务面板的「停止」是真能停**：点它会真正中止运行中的任务（不再只是个摆样子 disabled 的按钮）。顶层的对话仍走原来的「暂停 / 取消」。

**重启不会把任务越搞越多**：以前每重启一次，中断的任务就被再恢复一遍（1 → 2 → 4 → 8 成倍增长），实测能把一批 8 段生成放大成每对话 13 份。现在恢复后会给源任务打上持久标记，重复副本只保留最新一条、其余标记为已接续；你主动点的 retry / resume 不受影响。
...
## 2.13 编辑 / 重新生成 / 分支：三个「重开」入口

鼠标移到任意一轮消息上，右下角会浮出一行小按钮。它们的共同点是**回到那个时间点重来一次**，所以先记住一条原则：**这一轮之后的消息会被删掉，之前的全部保留。**

![消息操作区：自己的消息是「编辑 · 分支」，AI 回复是「复制 · 重新生成 · 新会话」](images/35-message-actions.png)

| 按钮 | 出现在 | 干什么 |
|---|---|---|
| **编辑** | 自己的消息 | 在那条消息的位置就地展开编辑框，改完按同一条提问重发 |
| **分支** | 自己的消息 | 从这条消息分叉到**一个新会话**，旧会话原样不动 |
| **复制** | AI 回复 | 复制整条回复 |
| **重新生成** | AI 回复 | 拿这条回复对应的那句提问**原样再答一次**（一个字都不用改） |
| **新会话** | AI 回复 | 在这里划一条分割线，线以上的老消息不再发给模型（见 3.2） |

**为什么反复点「重新生成」不会越点越乱**

截断点落在**那条提问**上，而不是这条 AI 回复上。后果是：

- 它前面几轮的问答**逐字节不动**，DeepSeek 这类支持前缀缓存的模型继续命中缓存（省的就是这笔钱）；
- 反复点同一个按钮，被重发的始终是同一句提问，**不会像「从回复处截断」那样越点越积出一串重复提问**。

点完会提示「正在重新生成（复用前缀缓存）」；「编辑」成功后提示「已从该消息重开，编辑点之前的上下文将复用缓存」。

**「重新生成」什么时候弹确认框**

- **后面已经没有消息、输入框里也没草稿**：直接重跑，不打断你。
- **后面还有消息**：弹确认框，并写明「这条回复之后还有 N 条消息，会一并删除」。
- **输入框里有草稿**：同样会提醒「输入框里的草稿会被替换成这条提问」。

> ⚠️ **截断不可逆**：确认之后，那一轮之后的对话就真的删掉了。万一这一轮重发出错，删掉的部分不会自己长回来——所以不确定时，先用「分支」留一条退路。

**改字时这些东西不会丢**

- **引用**：编辑框里回填的是你当初输入的原样文本，`/ref` 技能引用和 `@` 工作区引用**一字不改**地跟着走。`@` 仍然在发送那一刻按这个会话的工作区替换成绝对路径，所以重发结果和第一次完全一致。
- **附件**：这一轮原来发的图片 / 文件会原样带回。**纯附件轮次**（只丢了个文件、一个字没写）也能重发，不会再被「内容不能为空」拦下来。

**编辑态下，底部输入区整体让位**

在会话里点「编辑」后，编辑框就长在那条消息上；底部输入框会锁住并显示：

> 正在编辑上面的消息：在编辑框里 Ctrl+Enter 确认，Esc 取消

同时底部发送键变成「**重新发送**」，可用性跟着编辑框里的字数走（清空就禁用），待发送的附件列表也暂时收起——免得你「以为在编辑旧消息，其实在写新消息」。

- `Ctrl+Enter`、或点编辑框里的「重新发送」= 确认这条路重开；
- `Esc`、或点「取消」= 放弃这次编辑，原文原样还在。

> **已知边界**：编辑态下**不能增删附件**，只能原样重发那一轮的附件。要换附件就别走「编辑」，直接开一轮新的。

> 会话正在回答的时候，「编辑」和「重新生成」会自动隐藏（和「分支」「新会话」一个口径），避免跟进行中的回复打架。这时候硬要用，会提示「请先等待当前回答结束或停止」。

**「分支」出来的是新会话**，会继承同一个 API 和模型名，分过去不用重新选模型。
...
3. 下拉里只有**当前 API 实际检测到的模型**，没有手写框——免得你填了一个目录里压根不存在的名字，发出去才报错。
4. 供应商后来上架了新模型？下拉末尾固定有一项「**↻ 重新检测模型**」，选它就当场重拉一次目录，**刷新前的选择会自动还原**，不必再进设置页点「检查模型」。它只是一次动作、不是模型选项，不会被写进会话、也不会发给服务端。
...
- **切 API 会清空模型名**：顶栏换 API 后，模型名一并清空让你重新选（不同供应商的模型名不一样，留着没意义），**也不会再偷偷回退到配置里的那个默认模型**。
- **没选过就不替你选**：会话还没明确选过模型时，下拉显示禁用占位项「**请选择模型…**」，要你自己挑一次——以前会默认选中目录里的第一项，而那个顺序只是供应商接口的返回顺序，没有任何含义，等于悄悄替你决定了对答的模型。
- **目录里没有但以前选过**：旧会话保存的模型名如果不在当前目录里，会置顶显示成「**已保存：xxx（请重新检查模型）**」提醒你，但不会替你选中它。
- **目录拿不到就直说**：拉取失败时下拉是空的、占位显示「请先在设置中检查模型」，发送也会被拦下来。**不再静默回退到 API 配置里的默认模型**——宁可不发，也不发到一个你没验证过的模型名上。
- **空目录算「还没查到」**：有些 API 在还没加载好时会返回空列表；以前空结果也算「查过了」，这一页就再也不重试，下拉永久卡住。现在改成**缓存里确实有模型才复用**，空了会自动重拉。
- **重拉失败会出声**：拿不到目录提示「无法获取模型目录：具体原因」，还没配 API 提示先去设置页，目录确实为空提示「该 API 当前没有可用模型」。
- **自动重新探测图像能力**：换 API 或换模型名后，`chat_supports_images` 重置为「待探测」，发送时按新模型的**真实能力**判断——换到纯文本模型不会再误发图片，换到多模态模型也不会再被当成纯文本。
- API 供应商卡片也瘦身了：只保留两行（名称 + 格式标签），模型名不再上卡片。

**注意**

> - **顶栏 = 选「去哪儿」（连接），输入区 = 选「派谁干」（模型）**，这两个是两个独立开关。
> - 升级会自动完成数据库迁移（给 `conversations` 表加 `model_name` 列），幂等、不会重复执行。
> - 切了会话但模型串了多半是切太快：已修掉「目录返回时会话已切换」导致旧会话模型串到新会话的问题。
> - **以前没主动选过模型的会话（含老会话），需要你在下拉里显式选一次才能发送**——这是有意为之，不替你把钱花到没挑过的模型上。
...
在任意一条 AI 回复的右下角点「**新会话**」（操作区三个按钮：复制 · 重新生成 · 新会话），就会在那条回复下面画出一条分割线：
...
- **卡片化**：供应商和 Agent 都改成卡片网格，点卡片才弹设置表单，右上角 × 删除。
...
- **删错了也能救回来**：点删除会先弹确认框，让你在「移到回收目录（可恢复）」和「彻底删除」之间二选一，并列出这个 Skill 被哪些 Agent 引用着（删除会一并移除引用）。回收掉的东西不会消失，而是落到设置页里那个「**回收目录**」区块，点一下就恢复；同页还有「已隐藏 Skill」列表兜住另一种情况。真要不留痕迹，再点「清空回收目录」——那时才是永久删除。
...
>
> **安装目录归属（修了一个狠的数据事故）**：以前导入单个 `.md` 或顶层散放 `SKILL.md` 的 zip，定义文件会直接落在托管 Skills 目录的根上，**删这一个 Skill 会把整个目录搬走、所有 Skill 一起消失**。现在凡是直接落在导入根的上传都会先建独立子目录，删除前也会校验归属（越界一律拒绝）；升级时会自动把历史裸文件迁入同名子目录，引用不会断。
>
> **删 Skill 的三条退路**：① 普通删除是「移到回收目录」，设置页 Skills 管理下方能一键恢复；② 同一个 Skill 在好几处目录都有一份时（比如打包内置 + 托管副本），只搬走一份它会立刻从另一份复活——现在这类按 ID 统一隐藏，一次点掉即从列表消失且重启不回来，可在「已隐藏 Skill」里恢复；③ 「彻底删除」是把文件直接删掉，**不可恢复**，选之前会再问一次。
>
> **单文件不用再补 frontmatter**：单个 `.md`、或 zip 里顶层散放的 `SKILL.md`，没有 YAML frontmatter 也能装了——自动取第一个标题当名称、第一段正文当说明，正文原样保留。
...
- **ComfyUI 弄丢镜头会自动补救**：如果你重启了 ComfyUI 或清了队列，已提交的镜头会从 `/history` 和 `/queue` 里同时消失。以前程序把「查不到」当成「还在跑」，一路空转到超时（默认两小时），任务面板长期挂着「运行中 · 完成 5/8」不动；现在每轮会核对一次队列，确认被丢弃的镜头按预算（`max_resubmit`，默认 1）自动重新提交同一工作流接着跑，预算用尽则记为「ComfyUI 已丢失该任务」并正常收尾，不再无限空转。
...
| —— | **顶栏可一键收起**：收起后整条操作区让位，API 与 Agent 两个下拉并回第一行 |
...
手机上顶栏要挤下轮次、文件、任务、Skill、MCP、刷新这一串按钮，小屏很吃亏。现在它右上角多了个开关：

![手机端顶栏折叠](images/38-mobile-topbar-compact.png)

- **收起后**隐藏整条操作区（轮次 / 文件 / 任务 / Skill / MCP / 刷新临时让位），**API 与 Agent 两个下拉并回第一行**，屏幕高度全部还给会话区。
- 想用那些按钮时，点一下它下面那条细箭头就回来。
- 这个选择**记在本地**，刷新后还在；桌面端根本不渲染这个按钮。
...
- **出错自动重试**：HTTP 5xx（含 500）与 429/502/503/504 同等对待，按 1.5s / 3s 退避重试、最多 3 次；失败请求体会落盘到 `naiba-model-error-payload.json` 取证（超长字段截断、**不含 API Key**）。
- **Codex Responses 聚合回填**：只收到「事件汇总」没有逐字流也能正确拿到正文、思考和工具调用，不会被误判成空答复；这种格式下真出现空流会按瞬时故障重发。
...
> **「API」和「模型」是两个独立开关**：这一页配的是**连接**（去哪儿）。具体用哪个模型，在**输入框下方左边的「模型」下拉**里选（列表来自该 API 的模型目录），按会话记忆。详见 3.1 节。
>
> **卡片很紧凑**：每张卡片压成两行——名称 + 格式标签（当前使用的有角标），模型名不再上卡片，卡片高度从 116px 降到 88px。
...
>
> 「我的工具集」存在后端 `config.json`（不再存 localStorage），重启不会丢。
...
- **5xx 会自动重试**（1.5s / 3s，最多 3 次）；如果三次都失败，去看进程工作目录下的 `naiba-model-error-payload.json`（不含 API Key）。
...
- **下拉停在「请选择模型…」或发不出去**：这是现在的有意行为——会话没有明确选过模型时不替你选，必须你在下拉里挑一个。挑完就能发。
- **下拉停在「请先在设置中检查模型」**：当前 API 的模型目录没拉到（Key 不对、地址不通、供应商没开通这个模型），去「设置 → API 供应商」点「检查模型」。
- **新上架的模型列表里没有**：下拉末尾选一次「**↻ 重新检测模型**」当场重拉；别去设置页绕一圈。
- **显示「已保存：xxx（请重新检查模型）」**：这个会话以前存过的模型已经不在目录里了（供应商下线或改名）。先「检查模型」拿到目录，再重新选一个。
- **换了模型仍按旧能力走**：切 API 或换模型名会自动把图像能力重置为「待探测」，下一次发送按新模型的真实能力判断；如果还是不对，重开一次会话让它重新探测。
- **想让 A 会话用便宜模型、B 会话用强模型**：直接在各自的会话里选就行，模型名按会话记忆，互不干扰。

## 6.5 权限一直弹窗

- 先看档位：切到「**自动**」= 工作区内免问，工作区外（读也算）仍会问；「**完全**」= 不再询问，可访问工作区外（**慎用**）。
- **工作区里的文件也要问？** 看确认卡上写的「当前工作区：…」——那是判定用的目录。如果这个会话切换过工作区，以前会出现「判定按新工作区、实际读写按启动时的旧工作区」，导致工作区内的文件被判越界反复弹窗；现在判定与执行统一按**当前会话的工作区**，已经修掉了。
- 子 Agent 报「无法确定工作区目录」：同样属于这类问题，现已改为继承**当前会话**的工作区与审批档位（权限不会超出父级）。
- **还是搞不清为什么被判越界**：设环境变量 `NAIBA_DEBUG_PERMISSION=1` 后重启，每次权限判定都会写日志（执行器身份、请求的原始与解析后路径、当前工作区、允许根、命中结果），照着一查就知道。
- 工作区里指向外部的链接（junction / 符号链接）**按真实目标判定**，仍然算越界。
...
| 位置 | 按钮 | 作用 |
|---|---|---|
| 自己的消息 | **编辑** | 在原处改字后重发（引用与附件都带着走） |
| 自己的消息 | **分支** | 从这一条分叉到新会话（继承同一 API 与模型） |
| AI 回复 | **复制** | 复制整条回复 |
| AI 回复 | **重新生成** | 用那条提问原样再答一次（前面的上下文一字不动） |
| AI 回复 | **新会话** | 在此划分割线，线以上不再进模型上下文 |
...
| N 任务 | 看所有后台 Job（可在这里**停止**运行中的任务） |
| N Skill | 看当前会话已选的 Skill |
| MCP | 圆点看连接状态（绿/红/黄/灰） |
| 刷新 | 整页 reload（内嵌窗口用不了 F5 时点它） |
| 收起/展开顶栏 | 仅手机端（≤760px）有：收起整条操作区，把屏幕还给会话区 |

### 底部状态栏

| 控件 | 默认 | 说明 |
|---|---|---|
| **模型** | 未选（显示「请选择模型…」） | 本会话用哪个模型；列表来自当前 API 的检测目录，必须显式选一次才能发送；末尾「↻ 重新检测模型」可当场重拉目录 |
