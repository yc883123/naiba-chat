# Naiba Chat 应用说明书（2.1.0 Beta）

> 本说明书面向「第一次打开 Naiba Chat」的小白读者，配图全部来自真实运行界面（2.1.0 Beta · 源码模式 · 截图 2026-09-10）。
...
1. **长会话不会越聊越贵（2.1.0 头牌）**：聊到上下文快满时划一条「新会话」分割线，线以上的老消息不再发给模型，**聊天记录一条不删**，还能让模型自己写交接文档再重置。
2. **一个不够就接一堆模型**：OpenAI 兼容 / Anthropic / Gemini / Ollama / LM Studio / llama.cpp / Unsloth……在线本地都能接。
3. **Skill：给 AI 装"行业操作手册"**：写小说、跑 ComfyUI、写视频提示词……用 `/` 一键把领域知识塞进对话。
4. **MCP：给 AI 插外设**：ComfyUI、文件、网页……只要 MCP 服务器在跑，AI 就能直接调用。
5. **ComfyUI 自动流水线**：批改工作流 → 批量提交 → 自动收图 → 缩略图预览 → 一句话复用到下一轮。
6. **RunningHub 云端生成**：没显卡也能出 420+ 类型的图、视频、音频、3D。
7. **并行与分身**：后台 Job + 子 Agent，任务互不阻塞，跑图的时候还能写稿。
8. **手机/局域网远程访问**：同 WiFi 扫码，躺床上也能用家里电脑。
9. **稳如老狗**：断线重连不重放、失败不丢已生成内容、5xx 自动重试、SHA-256 校验更新、可版本回退。
10. **文件面板 + 本轮修改文件**：AI 改过的文件直接在右侧看、可编辑、可存回磁盘，**改了什么一目了然**。
...
1. 去 GitHub Releases 页面下载 `naiba-chat-2.1.0-beta-windows-x64.zip`。
...
1. **填一个模型** → 顶栏 ⚙ 设置 → 「API 供应商」→ 点「添加 API」卡片 → 填地址 / Key / 模型名 → 点「检查」测连通。
...
| **底部状态栏** | 工作区下拉、审批档位、快捷消息 ｜ 右侧：思考强度、上下文百分比、就绪状态 |
...
- **交接文档快捷键**：快捷消息面板里内置了「写交接报告并 reset_context」预设，点一下插进输入框就行（见 3.2）。
...
**注意**：切换模型不影响会话历史；但**切 Agent 会改首条消息固化下来的工具集**（首轮注入，之后不可改）。

## 3.4 Skill：给 AI 装"行业操作手册"
...
- **5xx 自动重试**（2.1.0 新），失败请求体会落盘到 `naiba-model-error-payload.json` 便于定位。
...
- **本地 ComfyUI**（你有显卡）→ 3.6 节
- **云端 RunningHub**（没显卡）→ 3.7 节
