# Naiba Chat 应用说明书（2.1.1 Beta）

> 本说明书面向「第一次打开 Naiba Chat」的小白读者，配图全部来自真实运行界面（2.1.1 Beta · 源码模式 · 截图 2026-09-10）。
...
1. **🆕 2.1.1 每个会话单独选模型**：顶栏选「API」（去哪儿），输入区选「模型」（派谁干），**按会话记忆、分支继承**，换完还自动重新探测图片能力。
2. **长会话不会越聊越贵（2.1.0 头牌）**：聊到上下文快满时划一条「新会话」分割线，线以上的老消息不再发给模型，**聊天记录一条不删**，还能让模型自己写交接文档再重置。
3. **一个不够就接一堆模型**：OpenAI 兼容 / Anthropic / Gemini / Ollama / LM Studio / llama.cpp / Unsloth……在线本地都能接。
4. **Skill：给 AI 装"行业操作手册"**：写小说、跑 ComfyUI、写视频提示词……用 `/` 一键把领域知识塞进对话。
5. **MCP：给 AI 插外设**：ComfyUI、文件、网页……只要 MCP 服务器在跑，AI 就能直接调用。
6. **ComfyUI 自动流水线**：批改工作流 → 批量提交 → 自动收图 → 缩略图预览 → 一句话复用到下一轮。
7. **RunningHub 云端生成**：没显卡也能出 420+ 类型的图、视频、音频、3D。
8. **并行与分身**：后台 Job + 子 Agent，任务互不阻塞，跑图的时候还能写稿。
9. **手机/局域网远程访问**：同 WiFi 扫码，躺床上也能用家里电脑。
10. **稳如老狗**：断线重连不重放、失败不丢已生成内容、5xx 自动重试、SHA-256 校验更新、可版本回退。
11. **文件面板 + 本轮修改文件**：AI 改过的文件直接在右侧看、可编辑、可存回磁盘，**改了什么一目了然**。
...
1. 去 GitHub Releases 页面下载 `naiba-chat-2.1.1-beta-windows-x64.zip`。
...
3. **选这个会话要用的模型** → 顶栏「API」下拉选刚才加的连接，**输入框下方左边「模型」下拉选具体模型**（想用列表里没有的就选「手动输入模型名称…」自己填）。
...
| **底部状态栏** | **模型下拉**（本会话用哪个模型）、工作区下拉、审批档位、快捷消息 ｜ 右侧：思考强度、上下文百分比、就绪状态 |
...
3. 想用下拉里没有的模型 → 选「**手动输入模型名称…**」，直接填（最长 256 字符，失焦或回车自动保存）。
...
> **2.1.1 起「API」和「模型」分开了**：这一页配的是**连接**（去哪儿）。具体用哪个模型，在**输入框下方左边的「模型」下拉**里选，按会话记忆；下拉里没有就选「手动输入模型名称…」自己填。详见 3.1 节。
...
| 控件 | 默认 | 说明 |
|---|---|---|
| **模型** | 跟随 API 默认 | 本会话用哪个模型，按会话记忆；可选「手动输入模型名称…」 |
