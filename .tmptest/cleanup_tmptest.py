...
用法：
    .venv\\Scripts\\python.exe .tmptest\\cleanup_tmptest.py            # dry-run，只打印计划
    .venv\\Scripts\\python.exe .tmptest\\cleanup_tmptest.py --apply    # 先打包备份，再删除

配套规则见维护说明 §8.7（用户表示结束会话时先问、再按此清理）。安全边界：
- 删除前把「所有将被删除的脚本」压进 .tmptest/_archive/tmptest_scripts_<日期>.zip；
- 同时写 _archive/deleted_manifest.txt 记录被删文件与体积；
- 只动 .tmptest，不碰仓库任何已跟踪文件；_archive/ 永不清。
"""
...
zip_path = ARCHIVE / f"tmptest_scripts_{stamp}.zip"
if del_scripts:
...
lines = [f"# {stamp} 清理 .tmptest 记录", f"保留 {keep_count} 个；备份 {zip_path.name}", ""]
for path in sorted(del_scripts):
...
print(".tmptest 剩余：", ", ".join(sorted(p.name for p in TMP.iterdir())))
