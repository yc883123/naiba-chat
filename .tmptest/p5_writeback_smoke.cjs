...
// 前置：停掉源码 server 后先播种 —— python .tmptest/seed_media_message.py
// 运行：$env:NODE_PATH="<node_modules>"; node .tmptest\p5_writeback_smoke.cjs
// 说明：Node 侧用 execFileSync 调 .tmptest/p5_writeback_apply.py 模拟"Job 跑完"
...
(async () => {
  ...
  try {
    ...
    const raw = execFileSync(PYTHON, [path.join(ROOT, '.tmptest', 'p5_writeback_apply.py')], {
      cwd: ROOT, encoding: 'utf-8', timeout: 120000,
      env: { ...process.env, TEMP: path.join(ROOT, '.tmptest'), TMP: path.join(ROOT, '.tmptest'), PYTHONPATH: path.join(ROOT, '.tmptest') },
    });
