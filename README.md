...
- **上下文用量提醒**：输入框旁的上下文圆环随**每次模型请求**刷新（不再等整轮结束），达到「设置 → 运行设置 → 上下文提醒阈值」（默认 80%，0 = 关闭提醒）时弹窗提醒——**正在运行的会话**涨过阈值即弹，**已结束的会话**改为点发送时才弹（可「继续发送」，被拦下时草稿不清空）；距上次提醒再涨 5 个百分点才再次提醒，换会话后重新计算；弹窗建议让模型撰写交接文档并开始新会话。
