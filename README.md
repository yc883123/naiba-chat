...
- **文件上传流式化**：上传改为 multipart 流式直传（内存更省、真实传输进度、单文件可取消、多文件并行上限 3、80MB 前端前置校验）；上传内容自动去重（重复文件复用既有缓存）、按日分目录存储、移除待发送附件时自动清理未引用文件；上传前/上传中的图片缩略图不再请求空路径。**待发送附件在输入框上方以竖直列表显示**：一行一个文件、固定高度（约 4 行）后内部滚动，文件名过长自动截断（悬停看全名），图片附件显示预览缩略图（点击看大图）、其它文件显示文件图标，每行右侧可单独移除。
