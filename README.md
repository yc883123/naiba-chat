# Naiba Chat 1.7.8 Beta

Naiba Chat 是运行在 Windows 本机的通用 AI 自动化工作台。它把在线或本地模型、内置工具、后台任务、Skill、MCP、视觉路由和文件产物统一到一个对话界面中。

1.7.8 Beta 在 1.7.7 的富文本开关基础上迭代：新增「对话快捷系统提示词」预设管理，角色卡导入自动收录为快捷提示词，并让检查更新后自动选中新版本；1.7.7 Beta 已把「富文本」开关从「轻量选项」中移出为独立的行内开关、切换立即生效。
...
1. 下载 `naiba-chat-1.7.8-beta-windows-x64.zip`。
2. 解压到一个可写目录。
3. 运行 `naiba-chat.exe`。
...
- `naiba-chat-1.7.8-beta-windows-x64.zip`

更新器会验证清单中的仓库、提交、文件名和 SHA-256。下载文件还必须是有效的 Windows 可执行文件；任何一项不一致都会终止安装。

## Beta 说明

这是 1.7.8 Beta，适合实际使用和反馈，但仍有以下边界：
...
$env:NAIBA_BUILD_VERSION = "1.7.8-beta"
