# Naiba Chat 1.7.3 Beta

Naiba Chat 是运行在 Windows 本机的通用 AI 自动化工作台。它把在线或本地模型、内置工具、后台任务、Skill、MCP、视觉路由和文件产物统一到一个对话界面中。

1.7.3 Beta 修复「可选回复」按钮与「本次对话规则」提示条重叠：AI 弹出选项按钮期间提示条保持隐藏让位，按钮移除后自动恢复显示。
...
1. 下载 `naiba-chat-1.7.3-beta-windows-x64.zip`。
2. 解压到一个可写目录。
3. 运行 `naiba-chat.exe`。
...
- `naiba-chat-1.7.3-beta-windows-x64.zip`

更新器会验证清单中的仓库、提交、文件名和 SHA-256。下载文件还必须是有效的 Windows 可执行文件；任何一项不一致都会终止安装。

## Beta 说明

这是 1.7.3 Beta，适合实际使用和反馈，但仍有以下边界：
...
$env:NAIBA_BUILD_VERSION = "1.7.3-beta"
