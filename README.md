...
- **工具与 Skill 选择器改版**：固定 Skill 改为卡片网格并与工具集等高；工具集分组标题/说明/计数一行呈现，「视觉」分组补说明并前移到「命令执行」之后；展开后的工具卡片更紧凑，只有展开的那一组标题条变浅灰以区分层次；修复列表在弹层里被压成一条线、点不动的问题。**工具集分区改为「卡片态 ↔ 编辑态」两态**：默认只显示 4 张只读预设卡（只读 / 标准 / ComfyUI 联动 / 全能，点一下整组套用并高亮）与「我的工具集」卡片，末尾是虚线「添加自定义工具集」卡；点卡片后卡片上移淡出、下方展开命名横条 + 搜索框 + 分组列表，选完可存成「我的工具集」复用（内置预设只读不可改，自定义工具集可删除）；卡片态弹层自动降到约 430px，进入编辑态回到 760px。
