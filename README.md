...
- **文件上传流式化**：上传改为 multipart 流式直传（内存更省、真实传输进度、单文件可取消、多文件并行上限 3、80MB 前端前置校验）；上传内容自动去重（重复文件复用既有缓存）、按日分目录存储、移除待发送附件时自动清理未引用文件；缓存超限自动清理（阈值默认 256MB、设置页可调/可关闭，只删未被消息引用的缓存，历史图片永不自动删除）；上传前/上传中的图片缩略图不再请求空路径。
