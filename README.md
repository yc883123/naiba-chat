...
- **同一窗口内「新会话」分割线**：每条完整 AI 回复末尾、「复制」右侧新增「新会话」按钮——在**任意位置**划一条分割线，**此线以上的消息不再进入模型上下文，线以下的消息仍保留在上下文中**（所以可以只裁掉前面跑题的部分、保留最近的进展），聊天记录一条不删、分割线可逐条撤销；允许多条分割线并存，靠时间与位置区分，最新那条决定当前上下文起点。工具侧（让模型自己调用重置）与交接文档校验在后续版本加入。
