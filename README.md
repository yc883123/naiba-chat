...
- **同一窗口内「新会话开始」**：输入区新增「新会话」按钮——在会话末尾落一条上下文边界，**下一轮起模型不再看到此前的消息**（聊天记录一条不删，仍留在界面上，随时可点分隔条上的「撤销」恢复），用来在长任务里换掉塞满的上下文而不用新建对话；分隔条形如「新会话开始 · 手动重置 · 时间」。工具侧（让模型自己调用重置）与交接文档校验在后续版本加入。
