...
- **时间线与用量可观测**：活动时间线规则定稿（思考块一律折叠、思考/工具严格物理序、最终答复固定时间线末尾、每次模型请求开始处以横线分割，与用量明细的请求序号对应）；流式期间消息尾端常驻用量框，每完成一次模型请求即刷新缓存命中率/tokens（最后一次请求口径），完成后显示本轮总耗时与完成日期，「请求明细」展开逐次请求的 token/命中率/耗时；会话顶部新增「首次请求上下文」折叠卡，可查看第一轮发送给模型的内容（完整系统提示词原文、工具 JSON 定义、生成参数）。
