...
- **设置页卡片化**：API 供应商与 Agent 管理改为卡片网格（一行最多三张、末尾固定「添加 API」/「新增 Agent」卡片、卡片右上角 × 删除），点卡片才弹出该条目的设置弹层并加载已有配置；字段与文案保持原样，只换展示方式。**默认 Agent 不再在卡片上标「默认」角标或加高亮**——默认项由顶栏 Agent 选择器体现，卡片上任何强调色都会被误读成「当前选中」。
