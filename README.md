...
1.1 Beta 的重点不是增加某一种固定工作流，而是让模型根据用户意图直接调用所需能力，持续执行到完成，并把过程状态和最终产物稳定地交还给用户。
...
1. 下载 `naiba-chat-1.1-beta-windows-x64.zip`。
2. 解压到一个可写目录。
3. 运行 `naiba-chat.exe`。
...
$env:NAIBA_BUILD_VERSION = "1.1-beta"
