...
- **纯附件发送**：输入框不写文字也能直接发送文件/图片（拖入、粘贴、附件按钮均可）；发送按钮按可用性变化——无可发送内容时灰暗禁用，可发送时恢复实心主色，上传未完成或上下文已满时禁用并给出原因提示；仅发附件时模型侧会明确收到「用户未输入文字，只发送了以下附件」+ 附件路径，不会凭空脑补用户诉求；纯附件首轮会话标题自动取首个附件名。
