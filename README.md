...
- **快捷消息面板**：输入区底部新增「快捷消息」按钮，点击展开常用提示词列表（**独立列表**，与「开始新对话」页的自定义指令互不影响，各自增删改）；列表默认按「使用次数 × 2 + 新鲜度加分」排序——常用条目稳定靠前，新添加的条目在 7/30/90 天内有曝光窗口不会永远沉底，并列时新增的在前；点击条目把内容插入输入框（可继续编辑再发送）并累计使用次数，行内 `✎` 编辑、`✕` 删除；面板在窄屏上自动贴边对齐，触屏下操作按钮常显。
