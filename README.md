# Naiba Chat 1.5.0 Beta

Naiba Chat 是运行在 Windows 本机的通用 AI 自动化工作台。它把在线或本地模型、内置工具、后台任务、Skill、MCP、视觉路由和文件产物统一到一个对话界面中。

1.5.0 Beta 的重点不是增加某一种固定工作流，而是让模型根据用户意图直接调用所需能力，持续执行到完成，并把过程状态和最终产物稳定地交还给用户。
...
- **流式渲染稳定**：代码块等特殊显示框随内容增长实时渲染在框内，不再在流式阶段被当作纯文本、等整段回复输出完才突然变成代码框；思考块与工具块按时间顺序交错显示，正文不再被清空吞掉。
...
1. 下载 `naiba-chat-1.5.0-beta-windows-x64.zip`。
2. 解压到一个可写目录。
3. 运行 `naiba-chat.exe`。
...
- `naiba-chat-1.5.0-beta-windows-x64.zip`

更新器会验证清单中的仓库、提交、文件名和 SHA-256。下载文件还必须是有效的 Windows 可执行文件；任何一项不一致都会终止安装。

## Beta 说明

这是 1.5.0 Beta，适合实际使用和反馈，但仍有以下边界：
...
$env:NAIBA_BUILD_VERSION = "1.5.0-beta"
