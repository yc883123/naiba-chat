...
class NaibaChatApp:
    def __init__(self):
        ...
        self.data_migration = migrate_legacy_data(PC)
        self.config = ConfigStore(CONFIG_PATH, paths=PC)
        ...
        for raw in self.config.data.get("skills_dirs", []):
            try:
                ...
                validate_skills_dir(resolved, app_dir=APP_DIR, public_dir=PUBLIC_DIR, data_dir=DATA_DIR)
    ...
    def import_legacy_data(self, source: Path) -> dict[str, Any]:
        ...
        report = _copy_legacy_data(source, PC)
        if report["config"]:
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def _resolve_skill_dest(self, body: dict[str, Any]) -> tuple[str, Path] | None:
        ...
        try:
            validate_skills_dir(dest, app_dir=APP_DIR, public_dir=PUBLIC_DIR, data_dir=DATA_DIR)
        except ValueError as exc:
            self._json({"error": str(exc)}, HTTPStatus.BAD_REQUEST)
            return None
